...
def classify_photo():
    # print("Classify Photo", file=sys.stderr)
    try:
        image = request.files.get("image", "")
        image = Image.open(image)
        if image == "":
            return {"error": "error sending image"}
        image = image.resize((30, 30))
        # image = np.expand_dims(image, axis=0)
        image = np.array(image)
        prediction = np.argmax(model.predict(np.array([image])), axis=-1)
        sign = classes[prediction[0] + 1]
        return {"sign_prediction": sign}
    except ValueError:
        # print(ValueError, file=sys.stderr)
        return {"error": "error generating prediction"}
